...
#' @param required List of required columns to look for in the widget content input
evaluate_widget_input <- function(widget, colnames, required) {
  for (col in required) {
    if (!any(colnames == col)) {
      msg <- sprintf("%s requires a \"%s\" column but is not present.", widget, col)
      stop(msg)
    }
  }
}
...
#' @param colnames Column names provided from the widget content input
#' @param required List of expected columns to look for in the widget content input
warning_widget_input <- function(widget, colnames, expected) {
  for (col in expected) {
    if (!any(colnames == col)) {
      msg <- sprintf("%s expects a \"%s\" column but is not present.", widget, col)
      warning(msg)
    }
  }
}
